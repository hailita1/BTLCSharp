...
as
begin
	declare @tien float, @maphieu varchar(50), @tongtien float
	select @tien=ThanhTien, @maphieu=MaPhieu
	from inserted
	select @tongtien=TongTien
	from PhieuDatBan where MaPhieu=@maphieu
	if(@tongtien=0 or @tongtien is null)
		set @tongtien=@tien
	else
...
as
begin
	declare @tien float, @mahdn varchar(50), @tongtien float
	select @tien=ThanhTien, @mahdn=MaHoaDonNhap
	from inserted
	select @tongtien=TongTien
	from HoaDonNhap where MaHoaDonNhap=@mahdn
	if(@tongtien=0 or @tongtien is null)
		set @tongtien=@tien
	else
		set @tongtien=@tongtien+@tien
...
begin
	...
	select @manl=MaNguyenLieu, @sl=SoLuong from inserted
	select @slnl=SoLuong from NguyenLieu where MaNguyenLieu=@manl
	if(@slnl is not null)
		update NguyenLieu set SoLuong=@sl+@slnl where MaNguyenLieu=@manl
	else
		update NguyenLieu set SoLuong=@sl where MaNguyenLieu=@manl
end
...
create trigger UpdateGiaNhapNL on ChiTietHoaDonNhap
for insert, update
as
begin
...
create trigger UpdateGiaBanNL on ChiTietHoaDonNhap
for insert, update
as
begin
	declare @manl varchar(50), @gia float
...
create trigger GiaMonAn on NguyenLieu_MonAn
for insert
as
begin
